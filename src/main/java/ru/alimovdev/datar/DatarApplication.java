...
Добавить заметку
Удалить клиента
Отправить сообщение (рег.)
Сообщение для всех (owner)
статистика(подписка)
3. Работа с базой специалистов ->
Зарегистрировать специалиста
Настройка графика работы
Удалить специалиста (owner)
4. Настройки ->
Часы работы
...
//TODO Устаревший вариант
           ...
           const specialistsIdList = tgAdministrator.specialistIdList.split("/").filter(id => id.length > 1).map(id => parseInt(id.trim()));
            ...
            Спасибо, теперь все работает как должно, лайк тебе! Давай еще немного доработаем функционал кода, который я выкладывал выше.
            Дело в том, сейчас высота столбцов column, которые генерирует function createColumn(date, appointments) является фиксированной,
            шкала .time-scale соответственно тоже фиксированного размера. Я хочу чтобы ты модифицировал код таким образом, чтобы высота column
            и шкалы .time-scale зависела от данных, полученных из поля String workTimeLength специалиста Specialist ,для которого сгенерированы
            столбцы column. Поле workTimeLength является строкой описывающей продолжительность рабочего дня специалиста, и имеет следующий вид
             как в примере: "8#17", где символ "#" служит разделителем для времени начала рабочего дня и времени окончания рабочего дня. Таким
             образом, для примера "8#17" мы получаем следующее значение: начало рабочего дня: 8 часов 00 минут, окончание рабочего дня:
             17 часов 00 минут. При расчете высоты столбца column и высоты шкалы .time-scale и разметки .time-label следует опираться на данные,
             полученные из поля workTimeLength. Обрати пожалуйста внимание вот на что: в данный момент на экране мобильных устройств высота column и
              шкалы .time-scale с разметкой .time-label, а так же позиционирование карточек .card синхронизированы с исключительной точностью,
              все отображается идеально, пиксель-в-пиксель. Нельзя портить такое идеально налаженное соотношение, надо его сохранить. А в
              иделе интерполировать и для отображения на десктопе - сейчас позиционирование карточек относительно разметки немного рассинхронизировано.
 */